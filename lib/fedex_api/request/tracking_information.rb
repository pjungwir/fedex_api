...
require 'fedex_api/tracking_information'
  ...
  module Request
    class TrackingInformation < Base
      ...
      def process_request
        ...
        if success?(response)
          ...
          if response[:track_reply][:duplicate_waybill].downcase == 'true'
            ...
            [options].flatten.map do |details|
              FedexApi::TrackingInformation.new(details)
            end
          end
        else
      ...
      def package_type_valid?
        FedexApi::TrackingInformation::PACKAGE_IDENTIFIER_TYPES.include? package_type
      end
